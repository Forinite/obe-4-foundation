        ...
        <div className="p-6 bg-white dark:bg-gray-800 rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 border border-gray-100 dark:border-gray-700">
            ...
            <div className="mb-6">
                ...
                <div className="space-y-2 pl-2 text-gray-700 dark:text-gray-300 md:text-base text-xs border-l-4 border-cyan-400/60 dark:border-cyan-500/50">
                    <p className="">{data.address1}</p>
                    <p className="">{data.address2}</p>
                </div>
            </div>

            {/* Contact & Social Info Grid */}
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {infoCards.map((item, index) => (
                    <div
                        key={index}
                        className="flex flex-col items-center justify-center rounded-xl overflow-hidden border border-gray-100 dark:border-gray-700 bg-gray-50/80 dark:bg-gray-900/40 hover:border-cyan-400/50 hover:shadow-lg transition-all duration-300 group"
                    >
                        <div
                            className={`w-full flex items-center justify-center py-4 ${item.color} transition-all duration-300 group-hover:scale-105`}
                        >
                            {item.icon}
                        </div>
                        <div className="w-full px-3 py-3 text-center">
                            ...
                            <p className="text-gray-800 dark:text-gray-100 text-[15px] font-medium truncate">
                                {item.value}
                            </p>
                        </div>
                    </div>
            ...
            <div className="mt-10">
                <div className="flex items-center gap-2 mb-4">
                    ...
                    <h5 className="text-lg font-semibold text-gray-800 dark:text-gray-100 tracking-wide">
                        Open Days
                    </h5>
                </div>
                ...
                <ul className="flex flex-wrap gap-4">
                    {data.openDays.map((day, index) => (
                        <li
                            key={index}
                            className="relative group flex flex-col items-center justify-between bg-gradient-to-br from-gray-50 to-white dark:from-gray-900/60 dark:to-gray-800/80 px-5 py-3 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-all duration-300 hover:scale-[1.02]"
                        >
                            {/* Left accent dot */}
